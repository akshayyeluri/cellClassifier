...
def make_vars_on_cpu(NCLASS):
    with tf.compat.v1.variable_scope(__name__):
        ...
        tf.compat.v1.get_variable(name="conv3_b", initializer=tf.constant(0.1, shape=[128]))
...
def model(inp, train=False):
    ...
    with tf.compat.v1.variable_scope(__name__, reuse=True):
        ...
        conv3_weights = tf.compat.v1.get_variable("conv3_w")
        conv3_biases = tf.compat.v1.get_variable("conv3_b")
    ...
    conv1 = tf.nn.conv2d(inp, conv1_weights, strides=[1,1,1,1], padding="VALID")
    relu1 = tf.nn.relu(tf.nn.bias_add(conv1, conv1_biases))
    pool1 = tf.nn.max_pool(relu1, ksize=[1, 6, 1, 1], \
                           strides=[1, 6, 1, 1], padding="VALID")
    conv2 = tf.nn.conv2d(pool1, conv2_weights, strides=[1,1,1,1], padding="SAME")
    relu2 = tf.nn.relu(tf.nn.bias_add(conv2, conv2_biases))
    pool2 = tf.nn.max_pool(relu2, ksize=[1, 16, 1, 1], \
                           strides=[1, 16, 1, 1], padding="VALID")
    conv3 = tf.nn.conv2d(pool2, conv3_weights, strides=[1,1,1,1], padding="VALID")
    relu3 = tf.nn.relu(tf.nn.bias_add(conv3, conv3_biases))
    pool3 = tf.nn.max_pool(relu3, ksize=[1, 10, 1, 1], \
                           strides=[1, 10, 1, 1], padding="VALID")
    size = pool3.get_shape().as_list()
    pool3_flat = tf.reshape(pool3, [size[0], size[1] * size[2] * size[3]])
    dense1 = tf.nn.relu(tf.matmul(pool3_flat, fc1_weights) + fc1_biases)
    if train:
