...
def make_vars_on_cpu(NCLASS):
    with tf.compat.v1.variable_scope(__name__):
        tf.compat.v1.get_variable(name="conv0_w", \
          initializer=tf.random.truncated_normal([5, 1, DEPTH, 16], stddev=0.1, seed=SEED))
        tf.compat.v1.get_variable(name="conv0_b", initializer=tf.zeros([16]))
        
        tf.compat.v1.get_variable(name="conv1_w", \
          initializer=tf.random.truncated_normal([5, 1, 16, 16], stddev=0.1, seed=SEED))
        tf.compat.v1.get_variable(name="conv1_b", initializer=tf.constant(0.1, shape=[16]))
        
        tf.compat.v1.get_variable(name="conv2_w", \
          initializer=tf.random.truncated_normal([5, 1, 16, 32], stddev=0.1, seed=SEED))
        tf.compat.v1.get_variable(name="conv2_b", initializer=tf.constant(0.1, shape=[32]))

        tf.compat.v1.get_variable(name="conv3_w", \
          initializer=tf.random.truncated_normal([2, 1, 32, 32], stddev=0.1, seed=SEED))
        tf.compat.v1.get_variable(name="conv3_b", initializer=tf.constant(0.1, shape=[32]))

        tf.compat.v1.get_variable(name="conv4_w", \
          initializer=tf.random.truncated_normal([5, 1, 32, 64], stddev=0.1, seed=SEED))
        tf.compat.v1.get_variable(name="conv4_b", initializer=tf.constant(0.1, shape=[64]))

        tf.compat.v1.get_variable(name="fc1_w", \
          initializer=tf.random.truncated_normal([50 * 64, 512], stddev=0.1, seed=SEED))
